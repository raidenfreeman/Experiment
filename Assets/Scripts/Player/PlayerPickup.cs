...
// Should not hold any reference to the item (you can get it from the anchor if needed)
    public class PlayerPickup : MonoBehaviour
    {
        ...
        internal float GetReachRadius
        {
            get { return reachRadius; }
        }

        [SerializeField] internal float reachAngle = 120f;

        [SerializeField] private Transform holdingAnchor;
        ...
        {
            get { return holdingAnchor.childCount > 0; }
        }

        private Transform heldItem
        {
            get { return holdingAnchor.GetChild(0); }
        }

        private Vector3 forwardReach
        {
            get { return transform.forward * reachRadius; }
        }
        ...
        void Update()
        {
            if ((InputManager.ActiveDevice?.Action3?.IsPressed ?? false) && !isHoldingItem)
            {
                ...
                GetSurfacesInRangeGroupedByAngle().Any(x =>
                    x.Any(y => y.GetComponent<IInteractibleSurface>()?.TryInteract() ?? false));
            }
        ...
        private void PlaceItem()
        {
            ...
            if (!surfaceGroups.Any(x => { return x.Any(y => y.TryPlaceItem(heldItem)); }))
            {
                DropItemOnGround();
            }
        ...
        private IEnumerable<IOrderedEnumerable<PlacementSurface>> GetSurfacesInRangeGroupedByAngle()
        {
            ...
            var collidersHit = Physics.OverlapSphere(transform.position, reachRadius, 1 << 8)
                .Where(x =>
                {
                    var angle = Vector3.Angle(this.forwardReach,
                        Vector3.ProjectOnPlane(x.transform.position - _transform.position, _transform.up));
            ...
            return collidersHit
                ...
                .GroupBy(x =>
                    (int) (Vector3.Angle(this.forwardReach,
                        Vector3.ProjectOnPlane(x.transform.position - _transform.position, _transform.up)) / angleStep))
...
// that lets you modify the "shieldArea" var in the WireArcExample.js
    ...
    public class DrawSolidArc : Editor
    {
        void OnSceneGUI()
        {
            ...
            Handles.DrawSolidArc(player.transform.position + Vector3.up * 0.3f, player.transform.up,
                player.transform.forward, player.reachAngle, player.GetReachRadius);
            Handles.DrawSolidArc(player.transform.position + Vector3.up * 0.3f, player.transform.up,
                player.transform.forward, -player.reachAngle, player.GetReachRadius);
            ...
            {
                Handles.DrawSolidArc(player.transform.position + Vector3.up * 0.3f, player.transform.up,
                    player.transform.forward, i, player.GetReachRadius);
                Handles.DrawSolidArc(player.transform.position + Vector3.up * 0.3f, player.transform.up,
                    player.transform.forward, -i, player.GetReachRadius);
