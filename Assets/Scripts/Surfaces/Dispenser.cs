...
namespace UnityBridge
{
    ...
    public class Dispenser : PlacementSurface
    {
        ...
        [SerializeField] private IPickableItem itemToDispense;
        ...
        private void Awake()
        {
            if (pickableItemToDispenseAddedOnAwake != null)
            {
                ...
                if (item == null)
                {
                    Debug.LogError(nameof(pickableItemPlacedOnAwake) +
                                   " does not contain a component that implements " + nameof(IPickableItem));
                    return;
                }
                else
                {
            ...
            {
                Debug.LogError(nameof(pickableItemPlacedOnAwake) + " has no value in " + this.gameObject.ToString() +
